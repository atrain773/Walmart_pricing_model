...
pivoted_sales_test_validation <- sales_test_validation %>%
  pivot_longer(
...
```{r}
grouped_data_test <- pivoted_sales_test_validation %>%
  group_by(cat_id, day) %>%
...
```{r}
# left join data 
grouped_data_test <- left_join(grouped_data_test, calendar, by = "day")
write.csv(grouped_data_test, file = "../data/grouped_data_test.csv", row.names = FALSE)
```
...
```{r}
# plot data
quantity_test_plot <- plot_ly(grouped_data_test, x = ~as.Date(date), y = ~total, color = ~cat_id, type='scatter', mode = 'lines') %>%
