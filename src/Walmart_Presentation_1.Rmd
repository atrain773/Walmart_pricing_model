...
```{r}
# plot data
infleuntial_hoiday_plot <- plot_ly(grouped_data, x = ~as.Date(date), y = ~total, color = ~cat_id, type='scatter', mode = 'lines') %>%
  layout(title = "Walmart Product Data",
         xaxis = list(title = "Date"),
         yaxis = list(title = "Quantity"),
         shapes = influential_holidays)

infleuntial_hoiday_plot <- add_trace(infleuntial_hoiday_plot, x = c(as.Date("2015-12-25")), y = c(0), mode = 'lines',
                                  type = 'scatter',
                                  line = list(color = 'red', dash = 'dash'),
                                  name = "Christmas", showlegend = TRUE)al
#infleuntial_hoiday_plot <- add_trace(infleuntial_hoiday_plot, x = c(as.Date("2015-11-26")), y = c(0), mode = 'lines',
                                  #type = 'scatter',
                                 # line = list(color = 'brown', dash = 'dash'),
                                 # name = "Thanksgiving", showlegend = TRUE)
  

infleuntial_hoiday_plot
```
